...
const CourseSyllabus = () => {
  ...
  const handleSyllabusClick = async (activeCourse) => {
    ...
    try {
      ...
      if (message === 1 && all_data.length > 0) {
        const syllabusData = all_data.reduce((acc, data) => {
          ...
          if (existingWeek) {
            ...
            acc.push({
              week: data.week,
              days: [{ topic: data.topic, file: data.file, description: data.description }],
            });
          }
      ...
      Swal.fire({ text: 'course specific syllabus not found', confirmButtonColor: 'rgb(20,184,166)' }).then(() => {
  ...
  const toggleAccordion = (weekIndex, dayIndex) => {
    ...
    if (isExpanded) {
      setExpandedDays(expandedDays.filter(day => day !== `${weekIndex}-${dayIndex}`));
    } else {
  ...
  return (
    <div className="p-5">
      <div className="flex flex-wrap -mx-3 gap-y-4">
        {courses && courses.length > 0 ? <div className="xl:w-[calc(100%-320px)] w-full px-3">
          ...
          <div className="p-5 rounded-2xl  shadow-[0px_5px_25px_0px_rgba(59,154,168,0.15)] bg-white custom-height">
            <div className="flex space-x-4">
              <button
                className={`px-4 py-2 rounded-full ${clickedBatchType === "syllabus"
              ...
              <button
                className={`px-4 py-2 rounded-full ${clickedBatchType === "batches"
                  ? "bg-[#35C69D] text-white"
                  : "bg-[#35C69D1A] text-[#35C69D]"
                  } `}
                        ...
                        >
                          <div className="flex flex-col mt-10 weekbox">
                            {weekData.days.map((day, dayIndex) => (
                              <div
                                key={dayIndex}
                                className={`flex pt-3 pb-3 pl-3 pr-12  mb-2 weeksinglebox smothanimation border rounded-lg bg-[#35C69D1A] ${expandedDays.includes(
                                  `${weekIndex}-${dayIndex}`
                                )
                                  ? "border-emerald-400"
                                  : ""
                              ...
                              >
                                <a href="#" className="dlbtn"><i class="fa fa-download" aria-hidden="true"></i></a>
                                ...
                                {expandedDays.includes(
                                  `${weekIndex}-${dayIndex}`
                                    ...
                                    <div className="p-2  rounded-b-lg">
                                      <p>Description:</p>
                                      <br></br>
                                      <div dangerouslySetInnerHTML={{ __html: formatTextWithLineBreaks(day.description) }} />
                                    </div>
                                  )}
                              </div>
                            ))}
        ...
        </div> : courses.length > 0 && !loading ? <div className="xl:w-[calc(100%-320px)] w-full px-3">
          <div style={{ flex: 1, display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh' }}>
            <p style={{ textAlign: 'center' }} className="font-small text-slate-500">Alass! There are no courses assigned to you at the time! For more information, please contact the management team at support.maangcareers.com.</p>
          </div>
        </div>:<></>}

...
const Submission = () => {
    ...
    const handleDrop = (e) => {
        ...
        // Check if files were dropped and update fileAdded state accordingly
        setFileAdded(droppedFiles.length > 0);

        // setFiles([...files, ...selectedFiles]);
        // setCheckboxes([...checkboxes, ...selectedFiles]);
    ...
    const handleDoneClick = () => {
        if (selectedWeek && files.length > 0) {
            setShowModal(false);
        } else if (files.length === 0) {
            Swal.fire({ text: 'Please select at least one file.', confirmButtonColor: 'rgb(20,184,166)', })
        } else {
            Swal.fire({ text: 'Please select both week and topic.', confirmButtonColor: 'rgb(20,184,166)', })
        }
        handleStartUpload();
          ...
          border-slate-300 border"
            ...
            {showModal && (
                <Modal
                    ...
                    onRequestClose={() => {
                        if (!selectedWeek) {
                            Swal.fire('Please select both week and topic.')
                        } else {
                            setShowModal(false);
                        }
                    }}
                        ...
                        <div className="flex items-center gap-x-2 ">
                            ...
                            >
                                ...
                                {projectData.map((project) => (
                                    <option key={project.project_id} value={project.topic_id}>

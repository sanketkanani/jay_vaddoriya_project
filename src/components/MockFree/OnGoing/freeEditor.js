...
const FreeEditor = ({ selectedQs, isRun, setIsRun, consolePanel,
    setConsolPanel, sectionId, submitData,
    setSubmitData, topicId, activeTag,
    setActiveTag, scroll, clickedItems,
    isClicked,
    setIsClicked, fetchFreeQuestionList }) => {
    ...
    const getCompilerData = async () => {
        ...
        try {
            ...
            const response = await fetch(`${ApiBaseURL}/free-course-management/practice-compiler-question-load-template/?question_id=${selectedQs.id}`, {
                method: "GET",
                headers: {
    ...
    const { test_cases: { data } } = selectedQs;
    useEffect(() => {
    ...
    const handleSubmitCode = (buttonClicked) => {
        ...
        fetch(`${ApiBaseURL}free-course-management/mock-compiler-question-attempt/`, {
            method: "POST",
            headers: {
    ...
    const fetchFreeSubmitResponse = async (main_data, buttonClicked) => {
        if (loggedIn) {
            const apiUrl = `${ApiBaseURL}free-course-management/mock-compiler-question-attempt-response/?main_data_id=${main_data}`;
            const token = JSON.parse(loggedIn).token;
            try {
        ...
        <>
            <div className="right-code">
                    ...
                    <div className="flex items-center">
                        <FormControl variant="standard" sx={{ m: 1, minWidth: 120 }}>
                            <Select value={activeLanguage} onChange={handleLanguageChange}>
                                {compilerData && compilerData.length > 0 && compilerData.map((compiler) => (
                                    <MenuItem
                ...
                <div className="right-code-body relative tab-screen:h-[calc(100%-40px)] tab-screen:flex tab-screen:flex-col">
                    ...
                    {consolePanel && <div className="code-output">
